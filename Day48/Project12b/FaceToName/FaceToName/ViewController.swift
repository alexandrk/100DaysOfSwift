...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
  ...
  override func viewDidLoad() {
    ...
    if let savedPeople = defaults.object(forKey: "people") as? Data {
      ...
      do {
        ...
        print("Failed to load people. Error: ", error
        )
      }
    }
  ...
  @objc func addNewPerson() {
    ...
    let ac = UIAlertController(title: "Do you want to use 'Photo Library' or 'Camera'?", message: nil, preferredStyle: .actionSheet)
    ac.addAction(UIAlertAction(title: "Photo Library", style: .default) { [weak self] _ in
      self?.presetImagePicker(sourceType: .photoLibrary)
    })
    ac.addAction(UIAlertAction(title: "Camera", style: .default) { [weak self] _ in
      self?.presetImagePicker(sourceType: .camera)
    })
  ...
  func presetImagePicker(sourceType: UIImagePickerController.SourceType) {
  ...
  func deletePersonAtIndex(index: IndexPath) {
    ...
    do {
      ...
      print("Unable to remove image file for person: \(person.name)\n\(person.image)")
    }
