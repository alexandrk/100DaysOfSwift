...
let rendered = renderer.image { ctx in
  ...
  let leftEye = CGRect(x: 250, y: 250, width: 100, height: 100)
  ctx.cgContext.addEllipse(in: leftEye)
  
  let rightEye = CGRect(x: 650, y: 250, width: 100, height: 100)
  ctx.cgContext.addEllipse(in: rightEye)
  ctx.cgContext.drawPath(using: .fillStroke)
  
  UIColor.brown.setFill()
  let mouth = CGRect(x: 350, y: 650, width: 300, height: 150)
